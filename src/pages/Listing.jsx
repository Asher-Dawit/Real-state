...
export default function Listing() {
  ...
  const fetchListing = async () => {
   ...
   return <main> 
    ...
    {error && (<p className='text-center my-7 text-red-500'>Error loading listing</p>)}
...
h
        ...
        items-center bg-slate-100 cursor-pointer">
         ...
         {copied && ( <p className="fixed top-[23%] right-[5%] z-10 rounded-md
          bg-slate-100 border p-2">Link Copied</p>)}
          <div className="flex flex-col max-w-4xl max-auto p-3 my-7 gap-4">
            ...
            <p className="flex items-center gap-2 mt-6 gap-2text-slate-600
            text-sm">
            ...
            <div className="flex gap-4">
               ...
               p-1 rounded-md">{listing.type === 'rent' ? 'For Rent' : 'For Sale'}</p>
               {
                  listing.offer && (
                     <p className="bg-green-900 w-full max-w-[200px] text-white text-center
                     p-1 rounded-md"> ${+listing.regularPrice - +listing.discountedPrice}</p>
                  ) 
               }  
          </div>   
